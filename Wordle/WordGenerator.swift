...
class WordGenerator {
  ...
  static func generateRandomWord() -> String? {
    // START YOUR CODE HERE
      // Ensure the array of possible words is not empty
              guard !possibleWords.isEmpty else {
                  return nil
              }
              
              // Get a random index within the range of possible words array
              let randomIndex = Int.random(in: 0..<possibleWords.count)
      let randomWord = possibleWords[randomIndex]
      
      // Print out the selected goal word
      print("Selected goal word: \(randomWord)")
